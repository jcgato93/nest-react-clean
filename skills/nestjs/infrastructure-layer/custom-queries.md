...
**Ubicación**: `src/modules/{module}/infrastructure/repositories/{module}.repository.impl.ts`
...
export class BusinessEstablishmentRepositoryImpl
  extends BaseRepositoryImpl<PrismaBusinessEstablishment, BusinessEstablishment>
  implements BusinessEstablishmentRepository {

  constructor(prisma: PrismaService) {
    super(prisma, (client) => client.businessEstablishment);
  }
  ...
  async getEstablishmentsWithCityByCompanyId(
    ...
    const [establishments, count] = await Promise.all([
      this.getRepo().findMany({
        ...
        select: {
          ...
          city: true, // Incluir relación (select e include se combinan en un solo objeto)
        },
      }),
    ...
    const items: BusinessEstablishmentListResponseDto[] = establishments.map((establishment) => {
...
- Prisma no tiene un equivalente a `findAndCount`: pedir `findMany` + `count` en paralelo con `Promise.all` (ambos deben recibir el mismo `where`)
- `select` e `include` son mutuamente excluyentes en la misma query de Prisma. Si necesitas seleccionar campos específicos **y** una relación, anida la relación dentro de `select`: `select: { id: true, name: true, city: true }` (como en el ejemplo). Si no usas `select` en absoluto, usa `include: { city: true }` para traer la relación junto con todas las columnas del modelo
- En `select`/`include`, especificar los campos u objetos necesarios como objeto — esto ya es igual que la notación de objeto de TypeORM, no cambia
...
@Injectable()
export class GetBusinessEstablishmentsUseCaseImpl
  implements GetBusinessEstablishmentsUseCase {
...
## Configuración de Relaciones en el Schema

### Agregar Relación en el Modelo

**Ubicación**: `prisma/schema.prisma`

```prisma
model BusinessEstablishment {
  // ... campos existentes ...

  cityCode String @db.VarChar(250)

  // Relación al final del modelo
  city     City   @relation(fields: [cityCode], references: [code])

  @@map("business_establishment")
}
...
- Colocar relaciones al final del modelo después de todos los campos escalares
- El lado "muchos" declara `@relation(fields: [...], references: [...])`; el lado "uno" (opcional) declara el array inverso, ej. `establishments BusinessEstablishment[]`
- El campo referenciado (`City.code` en este ejemplo) **debe** tener `@unique` — a diferencia de TypeORM, Prisma no permite crear la relación en el schema sin esa constraint
- Hacer el campo opcional con `?` en el tipo si la columna es nullable (ej. `cityCode String?`, `city City? @relation(...)`)

Ver [database-entities.md](./database-entities.md) para el detalle completo de tipos de campo y patrones de relación.

### Generar Migración

```bash
pnpm run prisma:migrate:dev --name AddRelationToModel
```

Si la migración generada hace algo inesperado (ej. detecta el rename de una columna como un `DROP` + `ADD` en vez de un `RENAME`), edita el archivo SQL en `prisma/migrations/<timestamp>_.../migration.sql` antes de aplicarla.

## Ejemplo Completo

Ver la implementación completa en:
- Schema: `prisma/schema.prisma` (modelo `BusinessEstablishment`)
- DTO: `src/modules/business-establishment/application/dtos/business-establishment-list-response.dto.ts`
- Repositorio Domain: `src/modules/business-establishment/domain/repositories/business-establishment.repository.ts`
- Repositorio Impl: `src/modules/business-establishment/infrastructure/repositories/business-establishment.repository.impl.ts`
- Use Case Domain: `src/modules/business-establishment/domain/use-cases/get-business-establishments.use-case.ts`
- Use Case Impl: `src/modules/business-establishment/application/use-cases/get-business-establishments.use-case.impl.ts`
...
export abstract class CompanyRepository extends BaseRepository<Company> {
  ...
  abstract getEconomyActivitiesByCompanyId(id: string): Promise<CompanyEconomyActivitiesDto>;
}
```
...
export class DashboardQueryRepository {
   ...
   * @param idUser - ID del usuario (opcional) para filtrar compañías
   * @param page - Opciones de paginación
   */
  async getListCompany(
    ...
    // Construir filtro con tipado fuerte (tipo generado por Prisma para el modelo)
    const where: Prisma.CompanyWhereInput = {};
    if (idUser) {
      where.userId = idUser; // Reemplazar `userId` por el campo real si aplica
    }
...
Para consultas que ninguna combinación de `select`/`include`/`groupBy` de Prisma puede expresar razonablemente, usa `PrismaService.$queryRaw` (template literal parametrizado) en vez de construir SQL con concatenación de strings — ver el manejo de identificadores dinámicos en [`audit.interceptor.ts`](../../common/interceptors/audit.interceptor.ts) para el patrón de `Prisma.sql` + `Prisma.raw` cuando el nombre de tabla/columna es dinámico pero no viene de input de usuario.
...
- [ ] Implementar query en repositorio infrastructure
  - [ ] Usar `select` con solo campos necesarios (o `include` si no hay `select`)
  - [ ] Combinar relaciones dentro de `select` si ya estás seleccionando campos específicos
  - [ ] Pedir `findMany` + `count` en paralelo con `Promise.all` (Prisma no tiene `findAndCount`)
...
- [ ] Agregar relación en el modelo de `schema.prisma` si es necesaria
- [ ] Generar y aplicar migración (`pnpm run prisma:migrate:dev --name ...`)
- [ ] Verificar que no hay errores de compilación con `get_errors`

## Errores Comunes y Soluciones

### Error: "Type 'X' is not assignable to type 'Decimal'"
**Causa**: Un campo `Decimal` de Prisma no acepta un `number` plano al escribir, y al leer devuelve una instancia `Decimal`, no un `number`
**Solución**: Convertir con `Number(entity.price)` en `mapToDomain`; al escribir, Prisma sí acepta `number` o `string` directamente en `data`, no hace falta envolverlo

### Error: "Argument `references` must refer to a unique criterion"
**Causa**: La columna referenciada en un `@relation` no tiene `@unique` (o no es la PK) en el schema
**Solución**: Agregar `@unique` al campo referenciado en el modelo relacionado — a diferencia de TypeORM, Prisma no permite crear la relación sin esa constraint, así que no hay forma de "solo lógica" aquí
...
**Causa**: Un `create`/`update` viola una constraint `@unique` del schema
**Solución**: Capturar `Prisma.PrismaClientKnownRequestError` con `error.code === 'P2002'` y traducirlo a una excepción de dominio/HTTP significativa, en vez de dejar que el 500 crudo llegue al cliente
...
4. **Optimización**: 
   - Usar `select`/`include` para traer solo campos necesarios
   - Usar paginación para grandes volúmenes de datos
   - Ejecutar queries en paralelo con `Promise.all()` cuando sea posible (incluyendo el par `findMany`/`count` de toda query paginada)

5. **Type Safety**: 
   - Siempre especificar tipos explícitos
   - Evitar usar `any` en cualquier parte del código
   - Aprovechar los tipos generados por Prisma (`Prisma.{Model}WhereInput`, `Prisma.{Model}CreateInput`, etc.) en vez de tipos manuales
